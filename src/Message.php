...
class Message extends BaseMessage
{
    ...
    public function buildMessage()
    {
        if ( isset($this->from, $this->subject) && ( (isset($this->textBody) && !empty($this->textBody)) || (isset($this->htmlBody) && !empty($this->htmlBody)) ) )
        {
            ...
            if ( $batchMode )
            {
                ...
                foreach ($this->personalizations as $envelope)
                {
                    if ( ! isset($envelope['to']) || empty($envelope['to']) )
                    {
                        ...
                        if ($this->getSendGridMail()->getPersonalizationCount() === 1) {
                            $personalization = $this->getSendGridMail()->getPersonalizations()[0];
                        } else {
            ...
            {
                $content = new \SendGrid\Mail\Content('text/plain', $this->textBody);
                ...
                $content = new \SendGrid\Mail\Content('text/plain', ' ');
                $this->getSendGridMail()->addContent($content);
            }

            if ( isset($this->htmlBody) && !empty($this->htmlBody) )
            {
                $content = new \SendGrid\Mail\Content('text/html', $this->htmlBody);
                $this->getSendGridMail()->addContent($content);
            }
